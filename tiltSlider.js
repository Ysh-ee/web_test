...
;( function( window ) {
	...
	function TiltSlider( el, options ) {
		...
		this.animEffectsOut = ['slideLeftOut'];
		this.animEffectsIn = ['slideLeftIn'];
		// the items
		this.items = this.el.querySelector('ul.slides').children;
		// total items
	...
	TiltSlider.prototype._addNavigation = function() {
		...
		for( var i = 0; i < this.itemsCount; ++i ) {
			inner += i === 0 ? '<span class="current">'+i+'</span>' : '<span>'+i+'</span>';
		}
